...
from core.portable import is_portable_mode, get_config_dir
...
def run_cli_backup(args):
    ...
    if args.restore:
        ...
        if result.get("success"):
            print(f"Backup created: {result.get('path')} ({result.get('files_count')} files)")
        else:
            print(f"Error: {result.get('error', 'Unknown')}")
...
Examples:
  ...
  python sorter.py --backup         Create configuration backup
  python sorter.py --restore FILE   Restore from backup file
